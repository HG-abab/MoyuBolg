<script setup>
  ...
  import { getTreeHoleList, updateTreeHole, deleteTreeHole, searchTreeHole } from '@/api/treehole';
  ...
  async function refreshFunc() {
    loading.value = true;
    await getTreeHoleList().then((res) => {
      if (res.code === 0) {
  ...
  async function onFinish(values) {
    ...
    await searchTreeHole(submitData).then((res) => {
      if (res.code === 0) {
        tabData.value = res.data;
        tabData.value.forEach((item) => {
          item.createTime = dayjs(item.createTime).format('YYYY-MM-DD HH:mm:ss');
  ...
  async function updateIsCheckFunc(id, isCheck, record) {
    ...
    await updateTreeHole({ id, isCheck }).then((res) => {
      if (res.code === 0) {
  ...
  function onDelete(ids) {
    if (ids) {
      ids.forEach(async (item) => {
        await deleteTreeHole(item).then(async (res) => {
          if (res.code === 0) {
