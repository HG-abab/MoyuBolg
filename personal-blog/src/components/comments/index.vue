...
import { getCommentCountByType, addComment } from '@/api/comment'
...
const addParentComment = async () => {
  ...
  const title = ref('')
  if (props.type == 1) {
    await getAllArticle(props.typeId).then(res => {
      if (res.code == 0) {
        title.value = res.data[0].articleTitle
      } else {
        ElMessage.error(res.message);
      }
    })
  } else {
    await getMessageList(props.typeId).then(res => {
      if (res.code == 0) {
        title.value = res.data[0].content
      } else {
          ...
          <div class="comment_content">
            <div class="comment_content_header">
              ...
              <div>
                ...
                <svg-icon @click="handleReply(item.id)" name="comment"
              ...
              <template v-if="item.children && item.children.length > 0">
                <div class="ChildComment" v-for="child in item.children" :key="child.id">
                  <template v-if="child.isCheck">
                    <div
                         style="display: flex;margin-top: 1rem;border-top: 1px solid var(--el-border-color);padding-top: 1rem;">
                      ...
                      <div class="comment_content">
                        <div class="comment_content_header">
                          ...
                          <div>
                            ...
                            <svg-icon @click="handleReply(child.id)" name="comment"
                                      style="margin:0 0.2em 0 0.5rem;cursor: pointer" />
                    ...
                    <div class="children_content_body">
                      <div class="chidren_name">
                        <div class="name">
                          <span>回复 </span>
                          <span style="color:red">@{{ findParentComment(Comments,
                            child.parentId).commentUserName}}</span>:
                        </div>
